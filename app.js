...
function getState(){
  // Return the global state object (single source of truth)
  return state;
}
...
function getTurnCode(roster, name, day){
  ...
  // New multi-month structure
  if(roster.months){
    const ym = yearMonthISO(dateISO);
    const m = roster.months[ym];
    if(!m) return "";
    return m.data?.[name]?.[String(day)] || "";
  }
  // Legacy flat matrix structure
  const techIndex = (roster.names || []).indexOf(name);
  if(techIndex < 0) return "";
  return roster.matrix?.[techIndex]?.[day - 1] || "";
}
...
function rosterHasDate(roster, dateISO){
  ...
  return roster.year === y && roster.month === m;
}
...
function ensureDaySector(tasksOrDateISO, dateISOorSector, sectorArg){
  // Support both: ensureDaySector(tasks, dateISO, sector) AND ensureDaySector(dateISO, sector)
...
function buildDailyAutoReport(){
  ...
  const otsToday = ots.filter(o => (o.assignedDate===dateISO) || (daySector.tray||[]).some(t=>t.type==="OT" && t.otId===o.id));
...
function createIncident(){
  ...
  const assignedTo = $("incAssignee")?.value.trim() || "";
  const incident = {
    id: uid(),
    ...
    status: "Pendiente",
    assignedTo,
    assignedDate: assignedTo ? ($("datePicker").value) : "",
  };
  ...
  $("incOwner").value = "";
  $("incNotes").value = "";
  if($("incAssignee")) $("incAssignee").value = "";
  mediaState.incidentPhotos = [];
  renderPhotoPreview("incident");
  if($("incQrStatus")) $("incQrStatus").textContent = "";
  renderIncidents();
  renderOTs();
  renderFilterChips();
...
function renderIncidents(){
  ...
  if(q) items = items.filter(i => objectMatchesQuery(i, q, ["title","location","owner","notes","priority","status"]));
  if(UIv5.incOpen) items = items.filter(i => (i.status||"Pendiente") !== "Cerrada");
  if(UIv5.incAssigned) items = items.filter(i => !!i.assignedTo);
  if(UIv5.incMine && UIv5.me) items = items.filter(i => i.assignedTo === UIv5.me);

  const kb = document.getElementById("incidentKanban");
  if(kb) renderKanbanBoard(kb, items, "inc");

  // legacy list (hidden by default)
  ...
  if(list && !list.classList.contains("hidden")){
    list.innerHTML = items.map((item) => {
      ...
      return `<div class="stackItem">
        <div class="cardTitleRow"><b>${escapeHtml(item.title)}</b> <span class="metaPill">${escapeHtml(item.priority)}</span></div>
        <div class="muted">${escapeHtml(item.dateISO)} · ${escapeHtml(item.location||"Sin ubicación")}</div>
        <div class="cardMetaRow">
          <span class="metaPill">📷 ${hasPhoto ? item.photos.length : "pendiente"}</span>
          ${pendingMedia ? `<span class="statusPill missing">Completar luego</span>` : ""}
        </div>
    ...
    for(const btn of list.querySelectorAll(".incidentStatus")){
...
function createOT(){
  ...
  const assignedTo = $("otAssignee")?.value.trim() || "";
  const newOT = {
    id: uid(),
    createdAt: new Date().toISOString(),
    sector: $("sectorSelect").value,
  ...
  $("otReportedBy").value = "";
  $("otData").value = "";
  if($("otPhotos")) $("otPhotos").value = "";
  if($("otAssignee")) $("otAssignee").value = "";
  mediaState.otPhotos = [];
  renderPhotoPreview("ot");
  if($("otQrStatus")) $("otQrStatus").textContent = "";
  renderOTs();
  renderFilterChips();
  toast?.("OT creada ✅");
}
...
function renderOTs(){
  const q = GLOBAL_SEARCH_Q;
  let items = [...state.ots];
  if(q) items = items.filter(o => objectMatchesQuery(o, q, ["title","area","reportedBy","data","status"]));
  if(UIv5.otOpen) items = items.filter(o => (o.status||"Pendiente") !== "Cerrada");
  if(UIv5.otAssigned) items = items.filter(o => !!o.assignedTo);
  if(UIv5.otMine && UIv5.me) items = items.filter(o => o.assignedTo === UIv5.me);

  const kb = document.getElementById("otKanban");
  if(kb) renderKanbanBoard(kb, items, "ot");
  ...
  if(list && !list.classList.contains("hidden")){
    list.innerHTML = items.map((item) => {
      return `<div class="stackItem">
        <div class="cardTitleRow"><b>OT:</b> ${escapeHtml(item.title)}</div>
        <div class="muted">${escapeHtml(item.area||"Sin área")}</div>
        <div class="stackActions">
          <button class="btn btn-small otAdvance" data-id="${item.id}" ${(item.step||0) >= 2 ? "disabled" : ""}>Avanzar etapa</button>
        </div>
      </div>`;
    }).join("") || `<div class="muted">Sin OT registradas.</div>`;
    for(const btn of list.querySelectorAll(".otAdvance")){
...
function closeDay(){
  ...
  setTasks(state.tasks);
  rerenderAll();
  window.toast?.(`Dia cerrado: ${sector}. Tareas limpiadas: ${doneCount}`);
}
...
function processCsv(){
  ...
  if(!csvText){
    window.toast?.("Pega un CSV primero.");
    return;
  }
  window.showLoading?.("Procesando cuadrante…");
  setTimeout(()=>{ _processCsvCore(sector, csvText); window.hideLoading?.(); }, 50);
}
function _processCsvCore(sector, csvText){
  // Detect year/month from header or use current date
  const dateISO = $("datePicker")?.value || todayISO();
  const {y: year, m: month} = isoToYMD(dateISO);
  ...
  for(let r=1; r<rows.length; r++){
    ...
    dayNums.forEach((day, i) => {
      const code = (rows[r][i+1] || "").trim().toUpperCase();
      if(code) dataMap[name][String(day)] = code;
    });
  }
  ...
  roster.months ||= {};
  roster.months[ym] = { data: dataMap, photo: "" };

  saveState();
  fillAssigneeSelects();
  initMeSelect?.();

  toast?.(`Cuadrante guardado para ${sector} — ${ym}`);
...
function resetAll(){
  const doReset = () => {
    ...
    rerenderAll();
    window.toast?.("Panel reiniciado");
  };
  if(window.showConfirm){
    window.showConfirm({
      title: "Reset TOTAL",
      msg: "Borrarà todos los cuadrantes y tareas. Esta acción no se puede deshacer.",
      icon: "🗑️", okText: "Sí, borrar todo", okClass: "btn-danger"
    }).then(ok => { if(ok) doReset(); });
  } else {
    if(!confirm("Reset TOTAL? Borrará cuadrantes y tareas guardadas.")) return;
    doReset();
  }
}
...
function applyShiftRequest(){
  ...
  const fromCode = getTurnCode(roster, fromTech, d) || "D";
  const toCode = getTurnCode(roster, toTech, d) || "D";

  // Update in whichever structure exists
  if(roster.months){
    const ym = yearMonthISO(dateISO);
    const m = roster.months[ym];
    if(m && m.data){
      if(m.data[fromTech]) m.data[fromTech][String(d)] = toCode;
      if(m.data[toTech]) m.data[toTech][String(d)] = fromCode;
    }
  ...
  for(const line of lines){
    ...
    for(let i=0; i<7; i++){
      ...
      if(roster.months){
        ...
        const m = roster.months[ym];
        if(m && m.data && m.data[name]) m.data[name][String(d)] = code;
      } else if(roster.matrix){
...
function bootstrap(){
  ...
  if(!state.rosters["Fontanería"] || !Object.keys(state.rosters["Fontanería"]?.months || {}).length){
    // Parse default CSV into new structure
    const sector = "Fontanería";
    const ym = "2026-02";
    const rows = parseCSV(DEFAULT_FONT_CSV);
    const header = rows[0];
    const dayNums = header.slice(1).map(x => Number(x));
    const names = [];
    const dataMap = {};
    ...
    state.rosters[sector] = { names, months: { [ym]: { data: dataMap, photo: "" } } };
    saveState();
  }
...
function initMeSelect(){
  ...
  const roster = state?.rosters?.[sector];
  const names = roster?.names ? [...roster.names] : [];
  const current = meSel.value || UIv5.me || "";
  meSel.innerHTML = '<option value="">— Yo (opcional) —</option>' + names.map(n=>`<option value="${escapeHtml(n)}">${escapeHtml(n)}</option>`).join("");
  if(current) meSel.value = current;
}
...
function openQuickUpdate(kind, id){
  ...
  const t = document.getElementById("quickUpdateTitle");
...
window.addEventListener("DOMContentLoaded", ()=>{
  const modal = document.getElementById("modalQuickUpdate");
  document.getElementById("btnCloseQuickUpdate")?.addEventListener("click", ()=> closeModal(modal));
...
function initRosterManager(){
  ...
  importInp?.addEventListener("change", async ()=>{
    ...
    const sector = document.getElementById("sectorSelect")?.value || "Electricidad";
    const ym = currentYM();
    window.showLoading?.("Importando archivo…");
    try{
...
function initAuthUI(){
  ...
  document.getElementById("btnLogout")?.addEventListener("click", async ()=>{
    const okLogout = window.showConfirm
      ? await window.showConfirm({ title: "Cerrar sesión", msg: "¿Seguro que quieres salir?", icon: "👋", okText: "Salir", okClass: "btn-primary" })
...
async function importFullBackup(file){
  ...
  const confirmed = window.showConfirm
    ? await window.showConfirm({ title: "Restaurar backup", msg: "Esto sobrescribirá TODO el panel. Esta acción no se puede deshacer.", icon: "⚠️", okText: "Restaurar", okClass: "btn-danger" })
    : confirm("Esto sobrescribirá TODO el panel. ¿Seguro?");
  if(!confirmed) return;
