...
// ---------- UI Enhancements (v2) ----------
const PANEL_UI = {
  KEY: "panel_ui_state_v2",
  state: (() => { try { return JSON.parse(localStorage.getItem("panel_ui_state_v2") || "{}"); } catch { return {}; } })(),
  save(){ localStorage.setItem("panel_ui_state_v2", JSON.stringify(this.state)); },
  toast(msg){ try{ if(window.toast) window.toast(msg); }catch{} },
  setBodyClass(cls, on){ document.body.classList.toggle(cls, !!on); },
  isToday(dateISO){ return dateISO === todayISO(); },
  activeShift(){ return currentShiftByHour(); },
  animateKPI(elId, value){
    const el = document.getElementById(elId);
    if(!el) return;
    // If KPI has <b>, animate that. Otherwise animate the span itself.
    const target = el.querySelector("b") || el;
    const num = Number(String(value).replace(/[^\d.-]/g,"")) || 0;
    if(window.animateNumber) window.animateNumber(target, num);
    else target.textContent = String(num);
    if(window.kpiFlash) window.kpiFlash(target);
  }
};
...
function renderSectorSummaries(state, dateISO){
  ...
  for(const sector of ["Electricidad","Fontanería"]){
    ...
    if(sector==="Electricidad"){
      ...
      // KPI animados
      PANEL_UI.animateKPI("elecFree", free);
      PANEL_UI.animateKPI("elecInc", incCount);
      PANEL_UI.animateKPI("elecLoad", avg);
    }else{
      ...
      // KPI animados
      PANEL_UI.animateKPI("fontFree", free);
      PANEL_UI.animateKPI("fontInc", incCount);
      PANEL_UI.animateKPI("fontLoad", avg);
    }
  }
...
function renderTechGrid(state, dateISO, sector){
  ...
  let orderedNames = [...roster.names].sort((a,b) => {
  ...
  // coordModeFilter: en Coordinador, si es HOY, muestra solo turno activo (+ guardia si está)
  if(PANEL_UI.state.coordMode && dateISO === todayISO()){
    const active = currentShiftByHour();
    orderedNames = orderedNames.filter((name) => {
      const code = getTurnCode(roster, name, d);
      const turno = primaryShift(code);
      const isGuard = guardName && name === guardName;
      return isGuard || turno === active;
    });
  }
...
function onDragStartTray(e){
  try{ e.currentTarget && e.currentTarget.classList.add("dragging"); }catch{}
...
function onDragOverSlot(e){
  try{ e.currentTarget && e.currentTarget.classList.add("drop-target"); }catch{}
  e.preventDefault();
  e.currentTarget.classList.add("over");
}

function onDragLeaveSlot(e){
  try{ e.currentTarget && e.currentTarget.classList.remove("drop-target"); }catch{}
...
// Global: quitar clase dragging
window.addEventListener('dragend', (e)=>{
  try{ document.querySelectorAll('.dragging').forEach(x=>x.classList.remove('dragging')); }catch{}
});
